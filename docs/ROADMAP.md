...
- [x] Tài liệu roadmap chi tiết (file này) + hướng dẫn Pha P chi tiết ([`PHASE_P_GUIDE.md`](PHASE_P_GUIDE.md)).
- [ ] Môi trường Python cài đặt (việc của bạn — xem checklist trong `PHASE_P_GUIDE.md`).
- [ ] 3 bộ sinh dữ liệu tổng hợp — code thật (hiện là stub có docstring/spec trong `src/pqrst/data/synthetic/`).
- [ ] 3 baseline wrapper — code thật (stub trong `src/pqrst/baselines/`).
- [ ] Script validate — code thật (stub trong `scripts/`).
